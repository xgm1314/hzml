...
class UserSerializerRoot(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'password', 'password2', 'is_superuser', 'username', 'email', 'is_staff', 'gender',
                  'date_joined', 'position', 'mobile', 'code', 'birthday', 'image', 'one_department']
        extra_kwargs = {
            'password': {
                'write_only': True,
                'help_text': '密码'
...
class UserSerializerOrdinary(serializers.ModelSerializer):
    password2 = serializers.CharField(max_length=32, write_only=True, label='确认密码')

    class Meta:
        model = User
        fields = ['id', 'password', 'password2', 'image']
        extra_kwargs = {
            'password': {
                'write_only': True
            }
...
class UserDetailSerializerRoot(UserSerializerRoot):
...
class UserDetailSerializerOrdinary(UserSerializerOrdinary):
